...
phStatus_t phApp_Clif_Init( uint8_t *pTxBuffer, uint16_t wTxBufSize, uint8_t *pRxBuffer, uint16_t wRxBufSize);
...
extern void pWtoxCallBck(void);
#endif /* NXPBUILD__PHPAL_I14443P4MC_SW */

#ifdef NXPBUILD__PHPAL_I18092MT_SW
extern void pRtoxCallBck(void);
...
#ifdef NXPBUILD__PHHAL_HW_PN5180
phhalHw_Pn5180_DataParams_t sHalParams;
#endif
...
#ifdef NXPBUILD__PHHAL_HW_PN7462AU
phhalHw_PN7462AU_DataParams_t sHalParams;
#endif
...
#ifdef NXPBUILD__PHHAL_HW_PN5180
#ifdef PN5180_HAL_DIGITAL_DELAY
    if (sHalParams.wFirmwareVer >= 0x308)
    {
         DEBUG_PRINTF("\n!!!!ALERT!!! : With Pn5180 FW version >= 3.8, 'PN5180_HAL_DIGITAL_DELAY' macro needs to be disabled.!!!!\n");
         DEBUG_PRINTF("\n!!!!!!!!!!!! : This is not the right combination !!!!!!!!\n");
    }
#else /* PN5180_HAL_DIGITAL_DELAY */
    if (sHalParams.wFirmwareVer < 0x308)
    {
        DEBUG_PRINTF("\n!!!!ALERT!!! : With Pn5180 FW version < 3.8, 'PN5180_HAL_DIGITAL_DELAY' macro needs to be enabled.!!!!\n");
        DEBUG_PRINTF("\n!!!!!!!!!!!! : This is not the right combination !!!!!!!!\n");
    }
#endif /* PN5180_HAL_DIGITAL_DELAY */
...
/* Initialize the discover component */
    ...
    sDiscLoop.pHalDataParams          = &sHalParams;
#ifdef NXPBUILD__PHPAL_I14443P3A_SW
...
phStatus_t phApp_Clif_Init( uint8_t *pTxBuffer, uint16_t wTxBufSize, uint8_t *pRxBuffer, uint16_t wRxBufSize)
{
    ...
    /* Initialize the 523 HAL component */
...
/* Initialize the RC663 HAL component */
    ...
    PH_CHECK_SUCCESS_FCT(wStatus, phhalHw_PN7462AU_Init(&sHalParams,
...
phStatus_t phApp_ConfigureLPCD(void)
{
     ...
     */
        ...
        //status = phhalHw_Pn5180_Int_LPCD_GetConfig(&sHalParams, wConfig, &wValue);
        status = phhalHw_Pn5180_Instr_WriteE2Prom(&sHalParams,bLPCD_Threshold_EEPROMAddress, &bLPCD_Threshold, 1 );
        CHECK_SUCCESS(status);
...
phStatus_t phApp_ConfigureLPCD(void)
{
    ...
    status = phhalHw_GetConfig(&sHalParams, PHHAL_HW_CONFIG_PN7462AU_LPCD_NEW_VALUE, &wLpcdValue);
    CHECK_SUCCESS(status);

    if (wLpcdValue != 0x00)
    {
        status = phhalHw_SetConfig(&sHalParams, PHHAL_HW_CONFIG_PN7462AU_LPCD_REF_VALUE, wLpcdValue);
        CHECK_SUCCESS(status);
    }
