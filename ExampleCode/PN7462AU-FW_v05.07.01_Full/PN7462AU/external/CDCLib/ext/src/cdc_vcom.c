		...
		uint32_t event)
{
	...
	{
	case USB_EVT_OUT:
		pVcom->rx_count = hwUSB_ReadEP(hUsb, USB_CDC_OUT_EP, pVcom->rx_buff);
	...
	case USB_EVT_OUT_NAK:
		...
		{
			hwUSB_ReadReqEP(hUsb, USB_CDC_OUT_EP, pVcom->rx_buff,
					VCOM_RX_BUF_SZ);
			pVcom->rx_flags |= VCOM_RX_BUF_QUEUED;
		}
		break;
		...
		USBD_API_INIT_PARAM_T *pUsbParam)
{
	...
	ret = mwCDC_init(hUsb, &cdc_param, &g_vCOM.hCdc);
	...
	if (ret == LPC_OK)
	{
		...
		ret = mwUSB_RegisterEpHandler(hUsb, ep_indx, VCOM_bulk_in_hdlr,
				&g_vCOM);
		if (ret == LPC_OK)
		{
			/* register endpoint interrupt handler */
			ep_indx = ((USB_CDC_OUT_EP & 0x0F) << 1);
			ret = mwUSB_RegisterEpHandler(hUsb, ep_indx, VCOM_bulk_out_hdlr,
					&g_vCOM);
...
uint32_t vcom_bread(uint8_t *pBuf, uint32_t buf_len)
{
	...
	if (pVcom->rx_count)
	{
		...
		phHal_Nvic_DisableInterrupt(PHHAL_NVIC_HIF);
...
ErrorCode_t vcom_read_req(uint8_t *pBuf, uint32_t len)
{
	...
	hwUSB_ReadReqEP(pVcom->hUsb, USB_CDC_OUT_EP, pBuf, len);
	/* exit critical section */
	phHal_Nvic_EnableInterrupt(PHHAL_NVIC_HIF);
	pVcom->rx_flags |= VCOM_RX_DB_QUEUED;
...
uint32_t vcom_write(uint8_t *pBuf, uint32_t len)
{
			...
			&& ((pVcom->tx_flags & VCOM_TX_BUSY) == 0))
	{
		...
		/* enter critical section */
		phHal_Nvic_DisableInterrupt(PHHAL_NVIC_HIF);
		ret = hwUSB_WriteEP(pVcom->hUsb, USB_CDC_IN_EP, pBuf, len);
		/* exit critical section */
		phHal_Nvic_EnableInterrupt(PHHAL_NVIC_HIF);
	}
