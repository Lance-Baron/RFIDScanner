...
/** @file
 *
 * phExDoorAccess_Log.c : Facilitates the LOG feature while running Debug sessions.
 *
 * Project:  PN7462AU
 *
...
#if 0
            /* The LED is blinking to know that something is happening. No need to print continuously */
            if ( 0 == bPollingLoopEntryCount || 0 == ( bPollingLoopEntryCount % PHEXDOORACCESS_NOCARD_PRINTF_SKIP_COUNT)) {
                LOG_TXT("No Contactless card detected\n");
            }
#endif
            break;
...
void phExDoorAccess_Log_FlashBootEventIds(phFlashBoot_Event_Ids evtId) {
    switch (evtId)
    {
