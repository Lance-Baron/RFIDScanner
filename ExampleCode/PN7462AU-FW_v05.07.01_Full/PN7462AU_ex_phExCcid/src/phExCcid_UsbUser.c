...
/** @file
 *
 * phExCcid_UsbUser.c: <This File provides the USB User Specific Initializations>
 *
 * Project: PN7462AU
...
#include "phExCcid_Descriptors.h"
#include "phExCcid_Usb_If.h"
#include "phExCcid_UsbCcid.h"

/* USB Core Buffer */
PH_ALIGN(256) volatile uint8_t gphExCcid_Usb_CORE_Buffer[USBHAL_CORE_MEM_SIZE];
/* USB CCID Buffer */
PH_ALIGN(4) volatile  uint8_t gphExCcid_Usb_CCID_buffer[USBHAL_CCID_MEM_SIZE];

USBD_HANDLE_T UsbHandle;

const uint8_t gphExCcid_FSStringDescriptor[] =
{
    0x04,
    0x03,
...
ErrorCode_t USB_Configure_Event_Stub (USBD_HANDLE_T hUsb)
{
    ...
    gphExCcid_sUsb_Bus_Status.bAddressed = pCtrl->config_value;
    return LPC_OK;
}
...
{
    if (gphExCcid_sUsb_Bus_Status.bSuspendEnable == 0)
    {
        gphExCcid_sUsb_Bus_Status.bSuspendEnable = 1;
    }
...
{
    gphExCcid_sUsb_Bus_Status.bAddressed = 0;
    return LPC_OK;
}
...
ErrorCode_t USB_WakeUpCfg_Stub (USBD_HANDLE_T hUsb, uint32_t param1)
{
    ...
    if (param1)
        gphExCcid_sUsb_Bus_Status.bRemoteWakeupEnable = 1;
    else
        gphExCcid_sUsb_Bus_Status.bRemoteWakeupEnable = 0;
    return LPC_OK;
}
#endif
#endif
...
void phExCcid_Usbd_CCID_Init(void)
{
    ...
    {
        .intf_desc           = (uint8_t*)&(gphExCcid_FSConfigurationDescriptor.Ccid_Interface),
...
void phExCcid_Usbd_CORE_Init(void)
{
    ...
    USBD_API_INIT_PARAM_T usb_param =
    {
        ...
        .mem_base            = (uint32_t) gphExCcid_Usb_CORE_Buffer,
    ...
    DeviceDes.device_qualifier = (uint8_t*)&gphExCcid_DeviceQualifierDescriptor;
#else
