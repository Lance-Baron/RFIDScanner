...
*******************************************************************************/
uint8_t bHalBufferTx[PH_ISO10373_MAX_TXBUFFSIZE] = {0x00};   /* HAL TX buffer */
uint8_t bHalBufferRx[PH_ISO10373_MAX_RXBUFFSIZE] = {0x00};   /* HAL RX buffer */
...
int main (void)
{
    do
    {
        ...
        wStatus = phApp_Clif_Init(bHalBufferTx, sizeof(bHalBufferTx), bHalBufferRx, sizeof(bHalBufferRx));
        CHECK_STATUS(wStatus);
        if(wStatus != PH_ERR_SUCCESS) break;

        /* Initialize Reader Library PAL/AL Components */
        wStatus = phApp_RdLibInit();
        CHECK_STATUS(wStatus);
        ...
        phOsal_ThreadCreate(&ISO10373PCD.ThreadHandle, &ISO10373PCD, &ISO10373_PCD_App, &sHalParams);
    ...
    )
{
    ...
    *pAtqbLen = bLocalAtqbLen;
    ...
    {
        bDsi = pLocalPupi[1] & 0x03;
        bDri = (pLocalPupi[1] & 0x0C) >> 2;
    }
...
static phStatus_t IBlockExchange(void)
{
          ...
          && !(memcmp(pRxData, EOT, sizeof(EOT)) == 0))
    {
        ...
        wTxDataLength = wRxDataLength - 2;
        memcpy(aTxData, pRxData, wTxDataLength);
    ...
    if((wRxDataLength == sizeof(EOT))
       && (memcmp(pRxData, EOT, sizeof(EOT)) == 0))
    {
    	wStatus = phpalI14443p4_Deselect(&spalI14443p4);
    }

    /* Reset Protocol */
    wStatus = phpalI14443p4_ResetProtocol(&spalI14443p4);
    CHECK_STATUS(wStatus);
