...
typedef struct USBD_CORE_API
{
  ...
  */
  ...
  void (*StallEp0)(USBD_HANDLE_T hUsb);
     ...
     /** \fn ErrorCode_t GetEpHandler(USBD_HANDLE_T hUsb, uint32_t ep_index, USB_EP_HANDLER_T *ep_handler, void **data)
   *  Function to get default EP handler and its parameter for a particular endpoint.
   *
   *  This function is called by the application layer to retrieve the default
   *  Endpoint Handler and its data parameter for a particular endpoint.
   *  \n
   *  \note This interface is provided so that the application could get the default handler, then register their own
     *        endpoint handler to do some special handling and then call the default handler.
   *
   *  \param[in] hUsb Handle to the USB device stack.
   *  \param[in] ep_index  Endpoint index. Computed as
     *                       - For OUT endpoints = 2 * endpoint number eg. for EP2_OUT it is 4.
   *                       - For IN endopoints = (2 * endpoint number) + 1 eg. for EP2_IN it is 5.
   *  \param[inout] ep_handler  Double Pointer to Endpoint event handler function which is updated in the function.
     *  \param[inout] data  Double Pointer to the data parameter that needs to be passed when calling the event handler.
   *  \return Returns \ref ErrorCode_t type to indicate success or error condition.
   *          \retval LPC_OK On success
   *          \retval ERR_API_INVALID_PARAM2  ep_index is outside the boundary ( < 2 * USBD_API_INIT_PARAM_T::max_num_ep).
   */
...
static INLINE uint32_t USB_IsConfigured(USBD_HANDLE_T hUsb)
{
    USB_CORE_CTRL_T* pCtrl = (USB_CORE_CTRL_T*) hUsb;
    return (pCtrl->config_value != 0);
}
...
extern ErrorCode_t mwUSB_RegisterClassHandler(USBD_HANDLE_T hUsb, USB_EP_HANDLER_T pfn, void* data);
extern ErrorCode_t mwUSB_RegisterEpHandler(USBD_HANDLE_T hUsb, uint32_t ep_index, USB_EP_HANDLER_T pfn, void* data);
extern ErrorCode_t mwUSB_GetEpHandler(USBD_HANDLE_T hUsb, uint32_t ep_index, USB_EP_HANDLER_T *ep_handler, void **data);
extern void mwUSB_SetupStage (USBD_HANDLE_T hUsb);
extern void mwUSB_DataInStage(USBD_HANDLE_T hUsb);
extern void mwUSB_DataOutStage(USBD_HANDLE_T hUsb);
extern void mwUSB_StatusInStage(USBD_HANDLE_T hUsb);
